...
<?php
  ...
  $library_dropdown[]=array('id' => 'All', 'text' => 'All Libraries');
  ...
  $order_status_query = "select distinct `Order Status` from librarySystemOrders order by `Order Status` desc";
...
<?php
  ...
  if(isset($_GET["regenerateCSV"])) {
   ...
   //define the query
   if ($_GET['library_system_name'] == "All") {
      $libsys_query_raw = "SELECT * from librarySystemOrders where  `Order Status` = '" . $_GET['order_status'] . "'"
      . " and `Order Date` >= '" . $sd ."'"
      . " and `Order Date` <= '" . $ed ."'";
   } Else {
   ...
   $headingline1 = $_GET['library_system_name'] . " " . $_GET['order_status'] . " Orders " . " From " . $_GET['start_date'] . " to " . $_GET['end_date'];
   fwrite($csvFile, $headingline1 . "\n\n");

   $headingline2 = "\"Library System\",\"Location\",\"Order Nbr\",\"Order Date\",\"Order Status\",\"Category\",\"Product Model\",\"Configuration Description\",\"Vendor Config Id\",\"Quantity\",\"Unit Price\",\"Total Price\"";
   ...
   while (!$libsys->EOF) {
    ...
    echo "<script type='text/javascript'>window.location.href=\"/librarySystemOrders.csv?cachebuster=".time()."\";</script>";
...
<form method="get" id="generateCSVForm" target="_blank" action="#">
  <input type="hidden" name="cmd" value="<?php echo FILENAME_STATS_LIBRARY_SYSTEM; ?>">
...
<!-- body_text //-->
    <td width="100%" valign="top"><table border="0" width="100%" cellspacing="0" cellpadding="2">
     ...
     <tr>
        <td class="main">
        <?php
         echo zen_draw_form('new_date', FILENAME_STATS_LIBRARY_SYSTEM, '', 'get') . 
         'Library System&nbsp;&nbsp;' . 
   ...
   <tr>
       <table border="0" width="100%" cellspacing="2" cellpadding="2">
       <tr><?php echo zen_draw_form('search', FILENAME_STATS_LIBRARY_SYSTEM, '', 'get'); 
